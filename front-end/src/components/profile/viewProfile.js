import React from 'react'
import Event from '../events/events.js'
export default function viewProfile() {
    return (
        <div class="container">
            <div class="colum1">
                <img src="https://www.w3schools.com/html/img_girl.jpg" alt="Girl in a jacket" width="250" height="300"></img><br></br>
                <label id="labelname" for="lname">NAME</label> <br></br>
                <input value="First Name" type="text" id="fname" name="firstname" disabled></input><br></br>
                <input value="Last Name" type="text" id="lname" name="lastname" disabled></input><br></br>
            </div>
            <div class="aboutme">
                <h1>About me </h1>
                <label for="email">Email</label> <br></br>
                <input value="Email" type="text" id="email" name="email" disabled></input><br></br>
                <label for="Biography"  >BIOGRAPHY</label> <br></br>
                <textarea id="w3review" name="w3review" disabled rows="4" cols="50">
                </textarea><br></br>
                <div class="link">
                    <a href="https://www.w3schools.com">
                        <img alt="W3Schools" src="https://github.githubassets.com/images/modules/logos_page/GitHub-Mark.png" width="50" height="50"></img></a>
                    <a href="https://www.w3schools.com">
                        <img alt="W3Schools" src="data:image/png;base64," width="50" height="50"></img></a>
                    <a href="https://www.w3schools.com">
                        <img alt="W3Schools" src="https://image.flaticon.com/icons/png/512/8/8800.png" width="50" height="50"></img></a>
                </div><br></br>
                <Event /> <br></br>
                <button type="button" onclick="">Edit Profile</button>
                <button type="button" onclick="">Clear Event</button>
            </div>

        </div>
    )
}
